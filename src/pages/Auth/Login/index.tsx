...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const data = { username, password }; // Prepare the data to be sent

    try {
      const response = await handleLogin(data); // Call the login function
      notification.success({
        message: "Login Successful",
        description: "Welcome back! You are now logged in.",
      });
      // setLocalStorage(response);
      localStorage.setItem("token", response); // Set the token in localStorage
      navigate("/home");
    } catch (error: any) {
      notification.error({
